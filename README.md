...
| 格式 | 用途 |
|---|---|
| ANSI 文本 | `fastfetch --file logo.txt`、终端 `cat` |
| fastfetch 配置 | 指向 ANSI 文件的 `.jsonc`，含那些不好查的设置 |
| Neovim Lua | dashboard-nvim / snacks.nvim / alpha.nvim 的高亮组 |
| PNG | 分享、README 配图 |
| HTML / SVG | 浏览器、矢量输出 |
...
合并时 `source` 不用改：导出时就已经写成绝对路径了（见下）。
...
| `logo.source` | logo 文件路径 | **相对路径按「当前工作目录」解析，不是配置文件所在目录**——所以导出的配置里写的是绝对路径。移动 logo 后要同步改这里 |
