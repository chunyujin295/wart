...
改图标时重新生成这两个文件即可：`.ico` 供 exe 用，256px 的 `.png` 供运行时用（原图 1100px，全尺寸解码出来是 5MB，而它从来不会被画得比几十像素大）。原图留了不对称的空白（左边 83px、右边 31px），所以生成时先裁到墨迹范围、居中放进正方形、再缩放——直接缩放会让图标偏一边。图标直接用 logo 的白底，不另外加背景方块或圆角。
